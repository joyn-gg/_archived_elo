            ...
            using (var db = new Database())
            {
                ...
                if (game != null)
                {
                    if (game.GameState == GameState.Picking)
                    {
                        ...
                        var remainingPlayers = LobbyService.GetMentionList(LobbyService.GetUserList(Context.Guild, queue.Where(x => team1.All(y => y.UserId != x.UserId) && team2.All(y => y.UserId != x.UserId)).Select(x => x.UserId)));
